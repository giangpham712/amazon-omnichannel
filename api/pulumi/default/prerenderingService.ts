...
class PageBuilder {
  ...
  constructor({ env, primaryDynamodbTable, elasticsearchDynamodbTable, bucket }: PreRenderingServiceParams) {
    const roleName = `${prefix}pre-rendering-service-lambda-role`;
    ...
    const render = new aws.lambda.Function(`${prefix}ps-render`, {
      role: this.role.arn,
      runtime: 'nodejs14.x',
    ...
    const flush = new aws.lambda.Function(`${prefix}ps-flush`, {
      role: this.role.arn,
      runtime: 'nodejs14.x',
      handler: 'handler.handler',
      timeout: 30,
    ...
    const queueAdd = new aws.lambda.Function(`${prefix}ps-queue-add`, {
      role: this.role.arn,
      runtime: 'nodejs14.x',
      handler: 'handler.handler',
      timeout: 30,
    ...
    const queueProcess = new aws.lambda.Function(`${prefix}ps-queue-process`, {
      role: this.role.arn,
      runtime: 'nodejs14.x',
      handler: 'handler.handler',
    ...
    const eventRule = new aws.cloudwatch.EventRule(`${prefix}ps-process-queue-event-rule`, {
      description: `Triggers "ps-process-queue" Lambda function that will process all queued prerendering jobs.`,
      scheduleExpression: 'rate(5 minutes)',
      isEnabled: true
    });

    new aws.lambda.Permission(`${prefix}ps-process-queue-event-rule-permission`, {
    ...
    new aws.cloudwatch.EventTarget(`${prefix}ps-process-queue-event-target`, {
      rule: eventRule.name,
