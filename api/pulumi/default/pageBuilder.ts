...
class PageBuilder {
  ...
  constructor({
    ...
    this.role = new aws.iam.Role(`${prefix}pb-update-settings-lambda-role`, {
      assumeRolePolicy: {
        Version: '2012-10-17',
        Statement: [
          {
    ...
    new aws.iam.RolePolicyAttachment(`${prefix}pb-update-settings-lambda-role-policy-attachment`, {
      role: this.role,
      policyArn: policy.arn.apply(arn => arn)
    });

    new aws.iam.RolePolicyAttachment(`${prefix}pb-update-settings-lambda-AWSLambdaBasicExecutionRole`, {
      role: this.role,
      policyArn: aws.iam.ManagedPolicy.AWSLambdaBasicExecutionRole
    });

    const updateSettings = new aws.lambda.Function(`${prefix}pb-update-settings`, {
      role: this.role.arn,
      runtime: 'nodejs14.x',
    ...
    this.exportPagesLambdaRole = new aws.iam.Role(`${prefix}pb-export-pages-lambda-role`, {
      assumeRolePolicy: {
        Version: '2012-10-17',
        Statement: [
          {
    ...
    const exportPagesLambdaPolicy = policies.getPbExportPagesLambdaPolicy(primaryDynamodbTable, bucket);

    new aws.iam.RolePolicyAttachment(`${prefix}pb-export-pages-lambda-role-policy-attachment`, {
      role: this.exportPagesLambdaRole,
      policyArn: exportPagesLambdaPolicy.arn.apply(arn => arn)
    });

    new aws.iam.RolePolicyAttachment(`${prefix}pb-export-pages-lambda-AWSLambdaBasicExecutionRole`, {
      role: this.exportPagesLambdaRole,
      policyArn: aws.iam.ManagedPolicy.AWSLambdaBasicExecutionRole
    });

    const exportPagesCombineLambda = new aws.lambda.Function(`${prefix}pb-export-pages-combine`, {
    ...
    const exportPagesProcessLambda = new aws.lambda.Function(`${prefix}pb-export-pages-process`, {
      role: this.exportPagesLambdaRole.arn,
      runtime: 'nodejs14.x',
      handler: 'handler.handler',
      timeout: 60,
    ...
    this.importPagesLambdaRole = new aws.iam.Role(`${prefix}pb-import-page-lambda-role`, {
      assumeRolePolicy: {
        Version: '2012-10-17',
        Statement: [
          {
    ...
    new aws.iam.RolePolicyAttachment(`${prefix}pb-import-page-lambda-role-policy-attachment`, {
      role: this.importPagesLambdaRole,
      policyArn: importPageLambdaPolicy.arn.apply(arn => arn)
    });

    new aws.iam.RolePolicyAttachment(`${prefix}pb-import-page-lambda-AWSLambdaBasicExecutionRole`, {
      role: this.importPagesLambdaRole,
      policyArn: aws.iam.ManagedPolicy.AWSLambdaBasicExecutionRole
    });

    const importPagesQueueProcess = new aws.lambda.Function(`${prefix}pb-import-page-queue-process`, {
    ...
    const importPagesQueueCreate = new aws.lambda.Function(`${prefix}pb-import-page-queue-create`, {
      role: this.importPagesLambdaRole.arn,
      runtime: 'nodejs14.x',
      handler: 'handler.handler',
      timeout: 60,
