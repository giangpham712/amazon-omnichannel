...
class SellingPartner {
  ...
  _validateEndpointsVersions(endpoints_versions) {
    ...
    if (invalid_endpoints.length) {
      throw new CustomError({
        ...
        message:
          'One or more endpoints are not valid. These endpoints don\'t exist: "' + invalid_endpoints.join('","') + '"'
      });
    }
    ...
    if (invalid_endpoints_versions.length) {
      throw new CustomError({
        ...
        message:
          'The provided version for the following endpoint(s) is not valid: "' +
          invalid_endpoints_versions.join('","') +
          '"'
      });
    }
    ...
    if (scope) {
      ...
      if (!valid_scopes.includes(scope)) {
        throw new CustomError({
          ...
          message:
            'Scope for requesting token for grantless operations is invalid. Please provide one of: ' +
            valid_scopes.join(',')
        });
      }
  ...
  async _validateAccessTokenAndRoleCredentials(scope) {
    ...
    if (!this._tokenExists(scope) || !this._role_credentials) {
      throw new CustomError({
        ...
        message:
          'Did you turn off "auto_request_tokens" and forgot to refresh the access token and/or role credentials and/or the scope for a grantless token?'
      });
    }
  }
    ...
    if (!endpoints[endpoint].__versions.includes(version)) {
      throw new CustomError({
        ...
        message:
          ...
          operation +
          '". Should be one of: "' +
          endpoints[endpoint].__versions.join('","') +
          '"'
      });
    }
  ...
  _constructExchangeBody(auth_code) {
    if (!auth_code) {
      throw new CustomError({
        ...
        message:
          'Please provide an authorization code received from a "getAuthorizationCode" operation to exchange it for a "refresh_token".'
      });
    }
    let body = {
  ...
  async refreshRoleCredentials() {
    let signed_request = new Signer(
      this._region,
      this._options.use_sandbox,
      this._options.user_agent
    ).signRoleCredentialsRequest(this._credentials.aws_user);
    ...
    if (
      json_res &&
      json_res.AssumeRoleResponse &&
      json_res.AssumeRoleResponse.AssumeRoleResult &&
      json_res.AssumeRoleResponse.AssumeRoleResult.Credentials
    ) {
  ...
  //   ...
  // },
  // headers: { // Optional: the input parameters added to the headers of the operation.
  //   ...
  // }
  ...
  async callAPI(req_params) {
    ...
    let signed_request = new Signer(this._region, this._options.use_sandbox, this._options.user_agent).signAPIRequest(
      token_for_request,
      this._role_credentials,
      req_params
    );
