...
class Policies {
  ...
  getPbExportPagesLambdaPolicy(primaryDynamodbTable: aws.dynamodb.Table, bucket: aws.s3.Bucket): aws.iam.Policy {
      ...
      policy: {
        ...
        Statement: [
          ...
          },
          {
            ...
            Effect: 'Allow',
            Action: 'es:*',
            Resource: [pulumi.interpolate`${elasticsearchDomain.arn}`, pulumi.interpolate`${elasticsearchDomain.arn}/*`]
          }
        ]
      }
    });
  ...
  getOrdersLambdaPolicy({
    ...
    return new aws.iam.Policy('OrdersPolicy', {
      description: '',
      policy: {
        Version: '2012-10-17',
        Statement: [
          ...
          },
          {
            ...
            Resource: [pulumi.interpolate`${notificationsQueue.arn}`, pulumi.interpolate`${errorsQueue.arn}`]
          },
          {
            Sid: 'PermissionForLambda',
            Effect: 'Allow',
  ...
  getInventorySyncServiceLambdaPolicy({
    primaryDynamodbTable,
    elasticsearchDynamodbTable
  }: {
    primaryDynamodbTable: aws.dynamodb.Table;
    ...
    return new aws.iam.Policy('InventorySyncServiceLambdaPolicy', {
      description: '',
      policy: {
        Version: '2012-10-17',
        Statement: [
